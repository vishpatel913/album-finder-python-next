...
def list_album(album_repo=Depends(get_album_repo), artist_repo=Depends(get_artist_repo)):
    return list_albums_with_artist(album_repo, artist_repo)


@router.get("/{album_id}", response_model=AlbumWithArtist)
def get_album(album_id: str, album_repo=Depends(get_album_repo), artist_repo=Depends(get_artist_repo)):
    try:
